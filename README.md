...
Welcome to **AI Dev Tasks**! This repository provides a collection of `.prompt.md` files designed to supercharge your feature development workflow within the VS Code editor. By leveraging these commands with VS Code's AI Agent, you can systematically approach building features, from ideation to implementation, with built-in checkpoints for verification.
...
2.  In VS Code's Agent chat, initiate PRD creation:

    ```
    Run Prompt -> create-prd.prompt
...
2.  In VS Code's Agent chat, use the PRD to create tasks:

    ```
    Run Prompt -> generate-tasks
    Use @MyFeature-PRD.md and create tasks
    ```
...
2.  In VS Code's Agent chat, tell the AI to start with the first task (e.g., `1.1`):

    ```
    Run Prompt -> process-task-list
    Please start on task 1.1 
    ```
    *(Important: You only need to reference `@process-task-list.prompt.md` for the *first* task. The instructions within it guide the AI for subsequent tasks.)*
    ...
    ![Example of starting on a task with process-task-list.prompt.md](https://pbs.twimg.com/media/Go6I41KWcAAAlHc?format=jpg&name=medium)
...
1.  **Clone or Download:** Get these `.prompt.md` files into your project or a central location where VS Code can access them.
2.  **Follow the Workflow:** Systematically use the `.prompt.md` files in VS Code's Agent chat as described in the 5-step workflow above.
3.  **Adapt and Iterate:**
    *   Feel free to modify the prompts within the `.prompt.md` files to better suit your specific needs or coding style.
